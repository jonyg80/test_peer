...
function processVideo (parameters: {
  ...
  return new Promise(async res => {
    ...
    const options = [ '-f', 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best', ...command.args, '--external-downloader', 'aria2c', '--external-downloader-args', '--dir='+program['tmpdir'], '--out=' + sha256(url) + '.mp4', '--max-connection-per-server=8']
    try {
      const youtubeDL = await safeGetYoutubeDL()
